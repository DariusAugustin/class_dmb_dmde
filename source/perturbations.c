...
//MANUEL
        class_store_double_or_default(dataptr,-tk[ppt->index_tp_delta_chi]/k2,ppt->has_source_delta_chi,storeidx,0.0);
//MANUEL
...
//MANUEL
      class_store_columntitle(titles,"d_chi",ppt->compute_dm_thermo);
//MANUEL
...
//MANUEL
      class_store_columntitle(titles,"t_chi",ppt->compute_dm_thermo);
//MANUEL
...
//MANUEL
    class_store_columntitle(titles,"-T_chi/k2",ppt->compute_dm_thermo);
//MANUEL
...
//MANUEL
      ...
      if (ppt->compute_dm_thermo == _TRUE_) {

        printf("    DM-baryon interactions is ON.\n -> chiDM perturbation parameters:\n");

        if (ppt->is_thetab_blind == _FALSE_)
          printf("\t (is_thetab_blind = FALSE): You have selected to have the baryon velocity theta_b to respond to the DM velocity theta_chi.\n");
        else
          printf("\t (is_thetab_blind = TRUE): You have selected to have the baryon velocity theta_b be blind (i.e. ignore) the DM velocity theta_chi.\n");

        if (ppt->tight_chib == _TRUE_) {

          class_test(pth->pow_n <= -3.,
                     ppt->error_message,
                     "tight_chib = TRUE and n<=-3. This should never happen.");

          printf("\t (tight_chib = TRUE): You have selected as initial condition theta_chi = theta_b for all wavenumbers.\n");
        }

        else
          printf("\t (tight_chib = FALSE): You have selected as initial condition theta_chi = theta_b only for those wavenumbers for which Rx(tau_ini) > aH(tau_ini), while for those where this inequality is violated the initial condition will be theta_chi = 0.\n");
      }
...
//MANUEL
  ppt->has_source_theta_chi = _FALSE_;
//MANUEL
...
//MANUEL
    if (ppt->perturbations_verbose > 1)
      printf("k[index_md_scalars][0] = %.5e, k[index_md_scalars][index_k-1 = %i] = %.5e\n", ppt->k[ppt->index_md_scalars][0], index_k-1, ppt->k[ppt->index_md_scalars][index_k-1]);//MANUEL: PRINT
//MANUEL
  }
    ...
    if (is_early_enough == _TRUE_) {
      ...
      if ((ppw->pvecback[pba->index_bg_a]*
          ...
          (k/ppw->pvecback[pba->index_bg_a]/ppw->pvecback[pba->index_bg_H] >
           ppr->start_large_k_at_tau_h_over_tau_k))//MANUEL: NOTE: LCDM case
...
//MANUEL
      /* Chi dark matter (IDM-b) */
      class_store_columntitle(ppt->scalar_titles,"delta_chi",ppt->compute_dm_thermo);
      class_store_columntitle(ppt->scalar_titles,"theta_chi",ppt->compute_dm_thermo);
//MANUEL
...
//MANUEL
    /* chi */
    class_define_index(ppv->index_pt_delta_chi,ppt->compute_dm_thermo,index_pt,1); /* chi density */
    class_define_index(ppv->index_pt_theta_chi,ppt->compute_dm_thermo,index_pt,1); /* chi velocity */
//MANUEL
    ...
    if (ppt->compute_dm_thermo == _TRUE_) {
      ...
      if (ppt->tight_chib == _TRUE_)
        ...
        if (pth->pow_n <= -3.)
          ...
          Rx = ppw->pvecthermo[pth->index_th_Rx];

          if (Rx > a_prime_over_a)
            tight = 1.;
          else
            tight = 0.;
        }
...
//MANUEL
      if (ppt->compute_dm_thermo == _TRUE_) {
        ...
        ppw->pv->y[ppw->pv->index_pt_theta_chi] = tight*ppw->pv->y[ppw->pv->index_pt_theta_b]; /* chi velocity *///MANUEL: NOTE: if n>-3 then theta_chi = theta_b, else theta_chi = 0
      }
//MANUEL
      /* interacting dark matter */
      if (pba->has_idm_dr == _TRUE_) {
...
//MANUEL-Darius
    /** -  added perturbation terms for idedm interaction for dark energy */


        class_call(background_w_fld(pba,a,&w_fld,&dw_over_da_fld,&integral_fld), pba->error_message, ppt->error_message);

        if (pba->use_ppf == _FALSE_) {
          ppw->pv->y[ppw->pv->index_pt_delta_fld] = ((3*(2*pba->xhi_ixc-1-w_fld)*(1+w_fld+pba->xhi_ixc))/(2*(12*pow(w_fld,2)-2*w_fld-3*w_fld*pba->xhi_ixc+7*pba->xhi_ixc-14)))* (-(ktau_two/3. * (1.-om*tau/5.)
        * ppr->curvature_ini * s2_squared));
          
          
          ppw->pv->y[ppw->pv->index_pt_theta_fld] = -((3*(k*tau)*(1+w_fld+pba->xhi_ixc))/(2*(2*w_fld+3*w_fld*pba->xhi_ixc-7*pba->xhi_ixc+14-12*pow(w_fld,2))))*( ktau_two/3. * (1.-om*tau/5.)
        * ppr->curvature_ini * s2_squared);
        }
...
//MANUEL
      else if (ppt->compute_dm_thermo == _TRUE_)
        delta_cdm = ppw->pv->y[ppw->pv->index_pt_delta_chi];
//MANUEL
      else
        delta_cdm=0.;
...
//MANUEL
      if (ppt->compute_dm_thermo == _TRUE_)
        velocity_tot += rho_m_over_rho_r*(ppw->pvecback[pba->index_bg_rho_chi]/rho_m)*ppw->pv->y[ppw->pv->index_pt_theta_chi]/(1.+rho_m_over_rho_r);
//MANUEL
...
//MANUEL
    /* chi contribution -- adding it here so that _cb is computed correctly */
    if (ppt->compute_dm_thermo == _TRUE_) {

      ppw->delta_rho += ppw->pvecback[pba->index_bg_rho_chi]*y[ppw->pv->index_pt_delta_chi]; // contribution to total perturbed stress-energy

      ppw->rho_plus_p_theta += ppw->pvecback[pba->index_bg_rho_chi]*y[ppw->pv->index_pt_theta_chi]; // contribution to total perturbed stress-energy
...
//MANUEL
    ...
    //MANUEL: NOTE: is this missing some terms for delta_rho_m, rho_m, rho_plus_p_theta_m, and rho_plus_p_m (see cdm and dcdm)?
    }
...
//MANUEL
    if (ppt->compute_dm_thermo == _TRUE_) {
      delta_x = y[ppw->pv->index_pt_delta_chi];
      theta_x = y[ppw->pv->index_pt_theta_chi];
    }
//MANUEL
...
//MANUEL
      if (ppt->compute_dm_thermo == _TRUE_) {
        delta_x -= 3. * pvecback[pba->index_bg_H]*pvecback[pba->index_bg_a]*alpha;
        theta_x += k*k*alpha;
      }
//MANUEL
      ...
      if (pba->has_ncdm == _TRUE_) {
        for(n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++){
          ...
          //MANUEL: CHECK: NOTE: I implemented this:
...
//MANUEL
    /* Chi dark matter (IDM-b) */
    class_store_double(dataptr, delta_x, ppt->compute_dm_thermo, storeidx);
    class_store_double(dataptr, theta_x, ppt->compute_dm_thermo, storeidx);
//MANUEL
    /* Non-cold Dark Matter */
    if ((pba->has_ncdm == _TRUE_) && ((ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_) || (ppt->has_source_delta_m == _TRUE_))) {
      for(n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++){
...
//MANUEL
  double delta_x,theta_x;
  double S,cx2,Rx,dRx,beta;
//MANUEL
...
//MANUEL
    if (ppt->compute_dm_thermo == _TRUE_) {
      ...
      delta_x = y[pv->index_pt_delta_chi];
      theta_x = y[pv->index_pt_theta_chi];

      S = (pba->Omega0_chi/pba->Omega0_b);

      cx2 = pvecthermo[pth->index_th_cx2];
      Rx = pvecthermo[pth->index_th_Rx];
      dRx = pvecthermo[pth->index_th_dRx];

      if (ppt->is_thetab_blind == _FALSE_)
        beta = (S/(1.+R))*(Rx/pvecthermo[pth->index_th_dkappa]);//a measure of how much does theta_b favor DM over photons
      else
        beta = 0.;
    }
    else
...
//MANUEL
    /** - ---> chi: defining it early, before the equations for theta_b', because dy[theta_chi] will appear in the tca expression for dy[theta_b] (see arXiv:1802.06788v3, after my corrections) */
    ...
    //MANUEL-Darius
    /** -   added perturbation terms for idedm interaction for the component of dm that interacts with baryons   */
      dy[pv->index_pt_delta_chi] = -(theta_x+metric_continuity) + (pba->Omega0_chi/(pba->Omega0_cdm+pba->Omega0_chi))*pba->xhi_ixc*a_prime_over_a *(pvecback[pba->index_bg_rho_fld]/(pvecback[pba->index_bg_rho_cdm]+pvecback[pba->index_bg_rho_chi]))* (y[pv->index_pt_delta_fld]-y[pv->index_pt_delta_cdm])+ (pba->Omega0_chi/(pba->Omega0_cdm+pba->Omega0_chi))*pba->xhi_ixc*a_prime_over_a *(pvecback[pba->index_bg_rho_fld]/(pvecback[pba->index_bg_rho_cdm]+pvecback[pba->index_bg_rho_chi]) )* (y[ppt->index_tp_theta_tot]+metric_continuity)/3 ;
        
      dy[pv->index_pt_theta_chi] = - a_prime_over_a*theta_x + metric_euler + cx2*k2*delta_x + Rx*(theta_b - theta_x);
    }
    //MANUEL-Darius
//MANUEL
...
//MANUEL
      if (((ppt->compute_dm_thermo == _TRUE_) && (ppt->is_thetab_blind == _FALSE_)))
        dy[pv->index_pt_theta_b] += S*Rx*(theta_x - theta_b);
//MANUEL
      ...
      //    +R*ppw->tca_slip)/(1.+R)
      //   +metric_euler;//MANUEL: ORIGINAL
//MANUEL
      dy[pv->index_pt_theta_b] = (-a_prime_over_a*theta_b +k2*(delta_p_b_over_rho_b+R*(delta_g/4.-s2_squared*ppw->tca_shear_g)) +R*ppw->tca_slip)/(1.+R+beta*R) +metric_euler*(1.+R)/(1.+R+beta*R);//MANUEL: NEW

      if (((ppt->compute_dm_thermo == _TRUE_) && (ppt->is_thetab_blind == _FALSE_))) {
        if (pth->sigma_0 > 0.) {//MANUEL: NOTE: To avoid dividing by 0. in dRx/Rx

          dy[pv->index_pt_theta_b] += ( ( S*Rx + R*beta*a_prime_over_a + (R*S)/(1.+R)*dRx/pvecthermo[pth->index_th_dkappa] )*(theta_x - theta_b) + R*beta*dy[pv->index_pt_theta_chi] )/(1.+R+beta*R) ;//MANUEL: NEWEST: 07/24/2020 w/ implicit theta_x' plugged in: uncomment ONLY if dy[theta_chi] has been defined early

          // dy[pv->index_pt_theta_b] += ( ( S*Rx + R*beta*(a_prime_over_a - Rx) + (R*S)/(1.+R)*dRx/pvecthermo[pth->index_th_dkappa] )*(theta_x - theta_b) + R*beta*(-a_prime_over_a*theta_x + metric_euler + cx2*k2*delta_x) )/(1.+R+beta*R);//MANUEL: NEWEST: 07/24/2020 w/ explicit theta_x' plugged in
        }
        else
          dy[pv->index_pt_theta_b] += 0.;
      }
//MANUEL
    }
...
//MANUEL
        /** - ---> adding terms to photon equations in the presence of chi */
        if ((ppt->compute_dm_thermo == _TRUE_) && (ppt->is_thetab_blind == _FALSE_))
          dy[pv->index_pt_theta_g] += S*Rx*(theta_x - theta_b)/R;
//MANUEL
      }
    }
...
//MANUEL-Darius
    /** -  added perturbation terms for idedm interaction for the component of dm that does not interacts with baryons     */
      if (ppt->gauge == newtonian) {
       dy[pv->index_pt_delta_cdm] = -(y[pv->index_pt_theta_cdm]+metric_continuity) + pba->xhi_ixc*a_prime_over_a *(pvecback[pba->index_bg_rho_fld]/(pvecback[pba->index_bg_rho_cdm]+pvecback[pba->index_bg_rho_chi]))
        * (y[pv->index_pt_delta_fld]-y[pv->index_pt_delta_cdm])
        + pba->xhi_ixc*a_prime_over_a *(pvecback[pba->index_bg_rho_fld]/(pvecback[pba->index_bg_rho_cdm]+pvecback[pba->index_bg_rho_chi])) * (y[ppt->index_tp_theta_tot]+metric_continuity)/3 ; /* cdm density */
      ...
      if (ppt->gauge == synchronous) {
        ...
        + pba->xhi_ixc*a_prime_over_a *(pvecback[pba->index_bg_rho_fld]/(pvecback[pba->index_bg_rho_cdm]+pvecback[pba->index_bg_rho_chi]))
        * (y[pv->index_pt_delta_fld]-y[pv->index_pt_delta_cdm])
        + pba->xhi_ixc*a_prime_over_a *(pvecback[pba->index_bg_rho_fld]/(pvecback[pba->index_bg_rho_cdm]+pvecback[pba->index_bg_rho_chi])) * (y[ppt->index_tp_theta_tot]+metric_continuity)/3 
        ;
      }
    }
//MANUEL-Darius
    ...
    /** -  added perturbation terms for idedm interaction for dark energy */
        ...
        -(1+w_fld)*(y[pv->index_pt_theta_fld]-metric_continuity) 
         -3.*(1-w_fld)*a_prime_over_a*(y[pv->index_pt_delta_fld]
          -(3.*(1+w_fld)+pba->xhi_ixc)*a_prime_over_a*a_prime_over_a*y[pv->index_pt_theta_fld]/k2) 
-pba->xhi_ixc*a_prime_over_a*(y[ppt->index_tp_theta_tot]+metric_continuity)/3
        ;
        dy[pv->index_pt_theta_fld] = /* fluid velocity */
          2.*a_prime_over_a*y[pv->index_pt_theta_fld]
	  +k2/(1.+w_fld)*y[pv->index_pt_delta_fld]
          +2.*a_prime_over_a*pba->xhi_ixc/(1.+w_fld)*y[pv->index_pt_theta_fld]
	  -pba->xhi_ixc*a_prime_over_a/(1.+w_fld)*0
          +metric_euler 
	  ;
...
/**
 * Compute the baryon-photon slip (theta_g - theta_b)' and the photon //MANUEL: NOTE: this line is wrong: the slip, as defined in this routine, is ACTUALLY (theta_b - theta_g)'. This is OK, since that is the expression that goes into the perturbation equations in the routines above.
