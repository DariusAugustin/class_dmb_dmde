...
//MANUEL-Darius
    if (pba->fluid_equation_of_state == CLP) {
      class_read_double("xhi_ixc",pba->xhi_ixc);
...
//MANUEL
//MANUEL-Darius
  /* --- DM-baryon scattering parameters */

  class_read_double("pow_n",pth->pow_n);// power of the velocity-dependent DM-baryon interaction cross section. See 1311.2937 and 1802.06788.
   class_call(parser_read_double(pfc,"mxlog",&param2,&flag1,errmsg),
             errmsg,
             errmsg);
  pth->mx = pow(10,param2);
 
  class_read_double("Vrms2",pba->Vrms2);// the RMS of the DM bulk velocity. CHECK: un-comment for writing it in the "thermodynamics" module.

  class_call(parser_read_double(pfc,"sigma_0",&param1,&flag1,errmsg),
             errmsg,
             errmsg);// reading the velocity-independent DM-baryon interaction cross section sigma_0 in cm^2
  class_call(parser_read_double(pfc,"sig0m",&param2,&flag2,errmsg),
             errmsg,
             errmsg);// reading the ratio of the cross section to the mass, sigma_0/mx, in cm^2/g

  class_test(((flag1 == _TRUE_) && (flag2 == _TRUE_)),
             errmsg,
             "In input file, pass either sigma_0 or sig0m, not both.");

  if (flag1 == _TRUE_)
    pth->sigma_0 = pow(10,param1)*1.e-4;// velocity-independent DM-baryon interaction cross section, in m^2. See 1311.2937 and 1802.06788.
  else if (flag2 == _TRUE_)
    pth->sigma_0 = (param2*(pth->mx*_MeV_over_kg_*1.e3))*1.e-4;// velocity-independent DM-baryon interaction cross section, in m^2. See 1311.2937 and 1802.06788.
    //MANUEL-Darius

  if ((pba->Omega0_chi != 0.) && (pth->sigma_0 != 0.)) {
    pth->compute_dm_thermo = _TRUE_;
    ppt->compute_dm_thermo = _TRUE_;
  }
  else {
  ...
  /** - baryon masses */
  class_call(parser_read_string(pfc,"baryon_mass",&string1,&flag1,errmsg),
             errmsg,
             errmsg);

  if (flag1 == _TRUE_) {
    flag2=_FALSE_;
    if (strcmp(string1,"hydrogen") == 0) {

      pth->baryon_mass=hydrogen;
      pth->mB = _m_H_;

      flag2=_TRUE_;
    }
    if (strcmp(string1,"electron") == 0) {

      pth->baryon_mass=electron;
      pth->mB = _m_e_;

      flag2=_TRUE_;
    }
    if (strcmp(string1,"proton") == 0) {

      pth->baryon_mass=proton;
      pth->mB = _m_p_;
    ...
    class_test(flag2==_FALSE_,
               errmsg,
               "could not identify 'baryon_mass' value, check that it is one of 'hydrogen', 'electron', or 'proton'");
  }

  pth->c_n = (pow(2.,(pth->pow_n+5.)/2.)*tgamma(3. + pth->pow_n/2.)/3./sqrt(_PI_));// n-dependent coefficient for the cross section (c_0 = 2.17). See 1311.2937 and 1802.06788.
  pth->mxb = ((pth->mx)/(pth->mx+pth->mB/_MeV_over_kg_));// = (muxb/mB)
  pth->muxb = (pth->mxb * pth->mB);//DM-b reduced mass [kg]

  class_call(parser_read_double(pfc,"sigmav_fo",&param2,&flag2,errmsg),
             errmsg,
             errmsg);
  if (flag2 == _TRUE_)
    pth->sigmav_fo = param2*1.e-6;// DM annihilation cross section (in m^3/s) that produces the thermal relic through freeze-out, <sigma v>.

  /** - Questions about the treatment of the baryon temperature Tb: */
  class_call(parser_read_string(pfc,
                                "is Tb blind to Tx",
                                &(string1),
                                &(flag1),
  ...
  if ((flag1 == _TRUE_) && (pba->Omega0_chi != 0.)) {
    if ((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL)) {
      pth->is_Tb_blind = _TRUE_;
      pth->early_blind_Tb = _TRUE_;
    }
    else {
      if ((strstr(string1,"n") != NULL) || (strstr(string1,"N") != NULL)) {
        pth->is_Tb_blind = _FALSE_;
        ...
        if (flag2 == _TRUE_) {
          if ((strstr(string2,"y") != NULL) || (strstr(string2,"Y") != NULL)) {
            ...
            if ((strstr(string2,"n") != NULL) || (strstr(string2,"N") != NULL)) {
              pth->early_blind_Tb = _FALSE_;
            }
            else {
              class_stop(errmsg,"incomprehensible input '%s' for the field 'early blind Tb'",string2);
            }
          }
        }
      }
      else {
        class_stop(errmsg,"incomprehensible input '%s' for the field 'is Tb blind to Tx'",string1);
      }
    }
  }

  /** - Questions about the treatment of the chiDM temperature Tx: */
  class_call(parser_read_string(pfc,
                                "realistic Tx",
                                &(string1),
                                &(flag1),
  ...
  if ((flag1 == _TRUE_) && (pth->compute_dm_thermo == _TRUE_)) {
    if ((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL)) {
      pth->realistic_Tx = _TRUE_;
    }
    else {
      if ((strstr(string1,"n") != NULL) || (strstr(string1,"N") != NULL)) {
        pth->realistic_Tx = _FALSE_;
      }
      else {
        class_stop(errmsg,"incomprehensible input '%s' for the field 'realistic Tx'",string1);
      }
    }
  }

  /** - Questions about including sound speed of chiDM: */
  class_call(parser_read_string(pfc,
                                "kill cx2",
                                &(string1),
                                &(flag1),
  ...
  if ((flag1 == _TRUE_) && (pth->compute_dm_thermo == _TRUE_)) {
    if ((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL)) {
      pth->pref_cx2 = 0.;// prefactor in front of extra term in cx2, could be 0 or 1; default: 1
    }
    else {
      if ((strstr(string1,"n") != NULL) || (strstr(string1,"N") != NULL)) {
        pth->pref_cx2 = 1.;// prefactor in front of extra term in cx2, could be 0 or 1; default: 1
      }
      else {
        class_stop(errmsg,"incomprehensible input '%s' for the field 'kill cx2'",string1);
      }
    }
  }

  /** - integrator for the reionization routine */
  class_call(parser_read_string(pfc,"reio_integrator",&string1,&flag1,errmsg),
             errmsg,
             errmsg);
  ...
  if (flag1 == _TRUE_) {
    ...
    if (strcmp(string1,"none") == 0) {
      ...
      pth->reio_integrator = no_int;

      flag2=_TRUE_;
    }

    if (strcmp(string1,"generic") == 0) {

      pth->reio_integrator = generic_int;

      flag2=_TRUE_;
    }
    if (strcmp(string1,"stiff") == 0) {

      pth->reio_integrator = stiff_int;
    ...
    class_test(flag2==_FALSE_,
               errmsg,
               "could not identify 'reio_integrator', check that it is one of 'none', 'generic', or 'stiff'");
  }

  /** - ignore helium */
  class_call(parser_read_string(pfc,"ignore helium",&string1,&flag1,errmsg),
             errmsg,
             errmsg);

  if (flag1 == _TRUE_) {
    if ((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL)) {
      pth->ignore_He = _TRUE_;
    }
    else {
      if ((strstr(string1,"n") != NULL) || (strstr(string1,"N") != NULL)) {
        pth->ignore_He = _FALSE_;
      }
      else {
        class_stop(errmsg,"incomprehensible input '%s' for the field 'ignore helium'",string1);
      }
    }
  }

  /** - photoionization in terms of photon temperature */
  class_call(parser_read_string(pfc,"photoionization with Tg",&string1,&flag1,errmsg),
             errmsg,
             errmsg);
  ...
  if (flag1 == _TRUE_) {
    if ((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL)) {
      ...
      if ((strstr(string1,"n") != NULL) || (strstr(string1,"N") != NULL)) {
        ...
        class_stop(errmsg,"incomprehensible input '%s' for the field 'photoionization with Tg'",string1);
      }
    }
  }
...
//MANUEL
  if (ppt->has_perturbations == _TRUE_) {
    ...
    if ((flag1 == _TRUE_) && (pba->Omega0_chi != 0.)) {
      if ((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL)) {
        ppt->is_thetab_blind = _TRUE_;
      }
      else {
        if ((strstr(string1,"n") != NULL) || (strstr(string1,"N") != NULL)) {
          ppt->is_thetab_blind = _FALSE_;
        }
        else {
          class_stop(errmsg,"incomprehensible input '%s' for the field 'is theta_b blind to theta_x'",string1);
        }
      }
    }
    ...
    if (pth->compute_dm_thermo == _TRUE_) {
      ...
      if (flag1 == _TRUE_) {
        if (pth->pow_n > -3.) {
          if ((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL))
            ppt->tight_chib = _TRUE_;
          else {
            if ((strstr(string1,"n") != NULL) || (strstr(string1,"N") != NULL))
              ppt->tight_chib = _FALSE_;
            else
              class_stop(errmsg,"incomprehensible input '%s' for the field 'tight chi-baryon ic'",string1);
          }
        }
        else//ignore what was read: if n <= -3. then we cannot use the tightly-coupled initial condition, period.
          ppt->tight_chib = _FALSE_;
      }
      else if (pth->pow_n <= -3.)
        ppt->tight_chib = _FALSE_;// if there was nothing to read but still n <= -3., the default is that we cannot use the tightly-coupled initial condition, period.
    }
  }
//MANUEL
...
//MANUEL
// reading precision parameters for chiDM (IDM-b)
class_read_int("refine",ppr->refine);
class_read_double("Rx_H_ratio",ppr->Rx_H_ratio);
class_read_double("dmde_buffer",ppr->dmde_buffer);
class_read_double("z_dmde_max",ppr->z_dmde_max);
class_read_double("tol_dmde_integration",ppr->tol_dmde_integration);
//MANUEL
...
//MANUEL
  ...
  pba->K = 0.;
  pba->sgnK = 0;
  pba->Omega0_lambda = 1.-pba->Omega0_k-pba->Omega0_g-pba->Omega0_ur-pba->Omega0_b-pba->Omega0_cdm-pba->Omega0_ncdm_tot-pba->Omega0_dcdmdr-pba->Omega0_idm_dr-pba->Omega0_idr-pba->Omega0_chi;//MANUEL
  pba->Omega0_fld = 0.;
  pba->a_today = 1.;
  pba->use_ppf = _TRUE_;
...
int input_try_unknown_parameters(double * unknown_parameter,
  ...
  if (pfzw->required_computation_stage >= cs_thermodynamics){
    ...
    // pr.recfast_Nz0 = 10000;//MANUEL: ORIGINAL: NOTE: commented out, since it caused problems with shooting in the case when recfast_Nz0 is changed from the default (as it is in the chiDM-b model)
...
int input_get_guess(double *xguess,
  ...
  for (index_guess=0; index_guess < pfzw->target_size; index_guess++) {
    ...
    case theta_s:
      ...
      break;
    case Omega_dcdmdr:
      Omega_M = ba.Omega0_cdm+ba.Omega0_idm_dr+ba.Omega0_dcdmdr+ba.Omega0_b +ba.Omega0_chi;//MANUEL
    ...
    case Omega_ini_dcdm:
      ...
      Omega_M = ba.Omega0_cdm+ba.Omega0_idm_dr+Omega0_dcdmdr+ba.Omega0_b +ba.Omega0_chi;//MANUEL
      gamma = ba.Gamma_dcdm/ba.H0;
      if (gamma < 1)
        a_decay = 1.0;
      else
