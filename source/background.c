 ...
 * * Manuel Buen-Abad, 2019-2020: include routines for dark matter interacting with baryons
 *
...
//MANUEL
  // Pass RMS value of the DM bulk velocity.//MANUEL:CHECK: change to the "thermodynamics" module ASAP.
  if (pba->has_Vrms2 == _TRUE_) {
    if (a_rel <= pow(10.,-3.))
      pvecback[pba->index_bg_Vrms2] = pba->Vrms2;
    else
      pvecback[pba->index_bg_Vrms2] = pba->Vrms2 * pow((pow(10.,-3.)/a_rel),2.);
  }
//MANUEL
  ...
  if (pba->has_cdm == _TRUE_) {
   ...
   //MANUEL-Darius
    ...
    pvecback[pba->index_bg_rho_cdm] = pba->Omega0_cdm * pow(pba->H0,2) / pow(a_rel,3) +   (pba->Omega0_cdm/(pba->Omega0_cdm+pba->Omega0_chi)) *  pba->Omega0_fld * (pow(pba->H0,2) / pow(a_rel,3) )* ( ( pba->xhi_ixc / (3*pba->w0_fld + pba->xhi_ixc  ) ) * (1- pow(a_rel,-3*pba->w0_fld- pba->xhi_ixc )));
    rho_tot += pvecback[pba->index_bg_rho_cdm];
    p_tot += 0.;
    rho_m += pvecback[pba->index_bg_rho_cdm];}
    else {
     pvecback[pba->index_bg_rho_cdm] = pba->Omega0_cdm * pow(pba->H0,2) / pow(a_rel,3) +   pba->Omega0_fld * (pow(pba->H0,2) / pow(a_rel,3) )* ( ( pba->xhi_ixc / (3*pba->w0_fld + pba->xhi_ixc  ) ) * (1- pow(a_rel,-3*pba->w0_fld- pba->xhi_ixc )));
    rho_tot += pvecback[pba->index_bg_rho_cdm];
    p_tot += 0.;
    rho_m += pvecback[pba->index_bg_rho_cdm];}
...
//MANUEL
  //chi
  /* (pba->Omega0_cdm/(pba->Omega0_cdm+pba->Omega0_chi)) *     (pba->Omega0_chi/(pba->Omega0_cdm+pba->Omega0_chi)) *      */
  if (pba->has_chi == _TRUE_) {
   pvecback[pba->index_bg_rho_chi] = pba->Omega0_chi * pow(pba->H0,2) / pow(a_rel,3)+ (pba->Omega0_chi/(pba->Omega0_cdm+pba->Omega0_chi)) *   pba->Omega0_fld * pow(pba->H0,2) / pow(a_rel,3) * ( (pba->xhi_ixc / (3*pba->w0_fld +pba->xhi_ixc ) ) * (1- pow(a_rel,-3*pba->w0_fld-pba->xhi_ixc)));
    rho_tot += pvecback[pba->index_bg_rho_chi];
    p_tot += 0.;//MANUEL: CHECK: this is assuming the chiDM (IDM-b) is never too hot/warm, so that there is only a negligible contribution to the pressure. This is important to corroborate.
...
/**
  pvecback[pba->index_bg_rho_chi] = pba->Omega0_chi * pow(pba->H0,2) / pow(a_rel,3)+ pba->Omega0_fld * pow(pba->H0,2) / pow(a_rel,3) * ( (pba->xhi_ixc / (3*pba->w0_fld +pba->xhi_ixc ) ) * (1- pow(a_rel,-3*pba->w0_fld-pba->xhi_ixc)));
    rho_tot += pvecback[pba->index_bg_rho_chi];
    p_tot += 0.;//MANUEL: CHECK: this is assuming the chiDM (IDM-b) is never too hot/warm, so that there is only a negligible contribution to the pressure. This is important to corroborate.
    rho_m += pvecback[pba->index_bg_rho_chi];
...
int background_w_fld(
  ...
  case CLP:
  //MANUEL-Darius
    *integral_fld = (3.*((1.+pba->w0_fld+pba->wa_fld)+pba->xhi_ixc)*log(1./a) + pba->wa_fld*(a/pba->a_today-1.)); 
   /**     *integral_fld = 3.*((1.+pba->w0_fld+pba->wa_fld)*log(pba->a_today/a) + pba->wa_fld*(a/pba->a_today-1.));   *integral_fld = (3.*((1.+pba->w0_fld+pba->wa_fld)+pba->xhi_ixc);   */
   //MANUEL-Darius
    break;
  case EDE:
  printf("hi2");
...
//MANUEL
  // -> Vrms2 of the DM
  class_define_index(pba->index_bg_Vrms2,pba->has_Vrms2,index_bg,1);
//MANUEL
  ...
  */
  ...
  if (pba->has_fld == _TRUE_){
    ...
    //MANUEL-Darius
     ...
     pvecback_integration[pba->index_bi_rho_fld] = rho_fld_today /pow(a,3*(1+pba->w0_fld)+pba->xhi_ixc);
...
//MANUEL
  class_store_columntitle(titles,"V_rms^2",pba->has_Vrms2);
//MANUEL
...
//MANUEL
    class_store_double(dataptr,pvecback[pba->index_bg_Vrms2],pba->has_Vrms2,storeidx);
//MANUEL
...
int background_derivs(
  ...
  //MANUEL-Darius
    ...
    dy[pba->index_bi_rho_fld] = -(y[pba->index_bi_a]*pvecback[pba->index_bg_H]*(3.*(1.+pvecback[pba->index_bg_w_fld])+pba->xhi_ixc)) * y[pba->index_bi_rho_fld];   
...
//MANUEL
    if(pba->has_chi){
      ...
      _class_print_species_("Chi Dark Matter (IDM-b)",chi);
